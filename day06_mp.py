...
class ThreadSafeDict:
    ...
    def __iter__(self):
        with self.lock:
            ...
            return iter(copy.deepcopy(self._dict))
...
def worker(task_queue, shared_dict, worker_index):
    print(f"Worker {worker_index} started")
    while True:
        new_mg: MapGame | None = None
        try:
            new_mg: MapGame | None = task_queue.get()
            ...
            print(f"Worker {worker_index} processing new map")
...
def part2(input: str):
    ...
    p = 1  # 1 = multiprocess, 0 = multithread

    manager = multiprocessing.Manager()
    task_queue = queue.Queue(0) if p == 0 else manager.Queue()
    shared_dict = ThreadSafeDict() if p == 0 else manager.dict()
    shared_dict['result'] = 0

    w_number = 40
    copy_args_list = [(task_queue, shared_dict, w_index) for w_index in range(w_number)]
    if p == 0:
        workers = [threading.Thread(target=worker, args=copy_args) for copy_args in copy_args_list]
    else:
        workers = [multiprocessing.Process(target=worker, args=copy_args) for copy_args in copy_args_list]
    for p in workers:
        p.start()
